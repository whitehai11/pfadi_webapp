...
export const scheduleReminders = () => {
    cron.schedule("*/30 * * * *", async () => {
        ...
        for (const rule of availabilityRules) {
            ...
            for (const event of events) {
                ...
                if (rule.min_response_percent !== null && rule.min_response_percent !== undefined) {
                    if (percent >= rule.min_response_percent)
                        continue;
                }
                ...
                const targets = missingUsers.filter((user) => {
                    if (rule.target_user_id && rule.target_user_id !== user.id)
                        return false;
                    if (rule.target_role && rule.target_role !== user.role)
                        return false;
                    return true;
                });
        ...
        for (const rule of packlistMissingRules) {
            ...
            const windowEnd = new Date(now.getTime() + leadTimeMs);
            const events = db
                .prepare("SELECT e.* FROM events e LEFT JOIN packlists p ON p.event_id = e.id WHERE e.packlist_required = 1 AND p.id IS NULL AND e.start_at BETWEEN ? AND ?")
                .all(windowStart.toISOString(), windowEnd.toISOString());
            for (const event of events) {
                if (rule.event_type && rule.event_type !== event.type)
                    continue;
        ...
        for (const rule of packlistIncompleteRules) {
            ...
            const windowEnd = new Date(now.getTime() + leadTimeMs);
            const events = db
                .prepare("SELECT DISTINCT e.* FROM events e JOIN packlists p ON p.event_id = e.id JOIN packlist_items i ON i.packlist_id = p.id WHERE i.status = 'missing' AND e.start_at BETWEEN ? AND ?")
                .all(windowStart.toISOString(), windowEnd.toISOString());
            for (const event of events) {
                if (rule.event_type && rule.event_type !== event.type)
                    continue;
