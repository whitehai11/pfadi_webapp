import { z } from "zod";
import { createInventory, deleteInventory, listInventory, updateInventory, getInventoryByTag } from "../services/inventory.service.js";
...
export const inventoryRoutes = async (app) => {
    ...
    app.put("/inventory/:id", { preHandler: requireMaterialOrAdmin }, async (request, reply) => {
        ...
        const item = updateInventory(request.params.id, parsed.data);
        if (!item) {
            return reply.code(404).send({ error: "Not found" });
        }
        return reply.send(item);
    ...
    app.delete("/inventory/:id", { preHandler: requireMaterialOrAdmin }, async (request, reply) => {
        const ok = deleteInventory(request.params.id);
        if (!ok) {
            return reply.code(404).send({ error: "Not found" });
        }
