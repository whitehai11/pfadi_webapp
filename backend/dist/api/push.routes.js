...
export const pushRoutes = async (app) => {
    ...
    app.post("/push/subscribe", { preHandler: [requireAuth, pushSubscribeRateLimit] }, async (request, reply) => {
        ...
        try {
            ...
            request.log.error({ err: error, userId: request.user?.id ?? null }, "Push subscribe failed");
    ...
    app.post("/push/unsubscribe", { preHandler: [requireAuth, pushUnsubscribeRateLimit] }, async (request, reply) => {
        ...
        try {
            ...
            request.log.error({ err: error, userId: request.user?.id ?? null }, "Push unsubscribe failed");
    ...
    app.post("/push/test", { preHandler: [requireAdmin, pushTestRateLimit] }, async (request, reply) => {
        try {
            ...
            request.log.error({ err: error, userId: request.user?.id ?? null }, "Push test send failed");
