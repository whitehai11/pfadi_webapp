...
import { createBox, deleteBox, getBoxByTag, isValidBoxTag, listBoxes, removeBoxMaterial, updateBox, upsertBoxMaterial } from "../services/box.service.js";
...
export const boxRoutes = async (app) => {
    ...
    app.put("/boxes/:id", { preHandler: requireMaterialOrAdmin }, async (request, reply) => {
        ...
        const box = updateBox(request.params.id, parsed.data.name, parsed.data.description ?? null);
        if (!box) {
            return reply.code(404).send({ error: "Nicht gefunden." });
        }
        return reply.send(box);
    ...
    app.delete("/boxes/:id", { preHandler: requireMaterialOrAdmin }, async (request, reply) => {
        const ok = deleteBox(request.params.id);
        if (!ok) {
            return reply.code(404).send({ error: "Nicht gefunden." });
        }
