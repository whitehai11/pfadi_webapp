...
const loginAttemptsByIpUser = new Map();
const loginAttemptWindowMs = 15 * 60 * 1000;
const loginAttemptMax = 8;
const getLoginAttemptKey = (ip, username) => `${ip}:${username}`;
const consumeLoginAttempt = (ip, username) => {
    const now = Date.now();
...
const clearLoginAttempt = (ip, username) => {
...
const serializeRefreshCookie = (token) => {
    const parts = [
...
const issueAccessToken = (app, payload) => app.jwt.sign(payload, { expiresIn: "15m" });
const createRefreshTokenRecord = (userId, rotatedFromTokenId = null) => {
    const token = generateRefreshToken();
    const tokenHash = hashRefreshToken(token);
    const now = new Date();
    const expiresAt = new Date(now.getTime() + refreshTokenMaxAge * 1000).toISOString();
    const id = randomUUID();
    db.prepare("INSERT INTO auth_refresh_tokens (id, user_id, token_hash, expires_at, created_at, rotated_from_token_id) VALUES (?, ?, ?, ?, ?, ?)").run(id, userId, tokenHash, expiresAt, now.toISOString(), rotatedFromTokenId);
...
export const authRoutes = async (app) => {
    app.post("/auth/register", { preHandler: registerRateLimit }, async (request, reply) => {
        ...
        try {
            ...
            const id = randomUUID();
            db.prepare("INSERT INTO users (id, email, password_hash, role, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)").run(id, normalized, passwordHash, role, status, now, now);
            ...
            request.log.error({ err: error, stack: error.stack }, "Register failed");
            throw error;
        }
    });
    app.post("/auth/login", { preHandler: loginRateLimit }, async (request, reply) => {
        ...
        try {
            ...
            request.log.error({ err: error, stack: error.stack }, "Login failed");
            throw error;
        }
    });
    ...
    app.post("/auth/logout", async (request, reply) => {
        ...
        const now = nowIso();
        if (providedToken) {
            db.prepare("UPDATE auth_refresh_tokens SET revoked_at = ? WHERE token_hash = ? AND revoked_at IS NULL").run(now, hashRefreshToken(providedToken));
        }
        try {
            await request.jwtVerify();
            const current = request.user;
            if (current?.id) {
                db.prepare("UPDATE auth_refresh_tokens SET revoked_at = ? WHERE user_id = ? AND revoked_at IS NULL").run(now, current.id);
            }
        }
        catch {
