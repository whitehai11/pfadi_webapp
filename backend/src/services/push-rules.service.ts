...
export const sendCustomPushRule = async (rule: Rule, options?: { updateLastSent?: boolean }) => {
  ...
  const now = new Date();
  const intervalWindowMs =
    rule.notification_type === "recurring" &&
    rule.is_recurring &&
    rule.interval_value &&
    rule.interval_unit
  ...
  for (const user of users) {
    const duplicate = db
      .prepare(
        "SELECT id FROM push_delivery_log WHERE rule_id = ? AND user_id = ? AND COALESCE(event_id, '') = '' AND sent_at >= ? LIMIT 1"
      )
      .get(rule.id, user.id, sinceIso) as { id: string } | undefined;
    if (duplicate) {
      skipped += 1;
      continue;
    }
  ...
  let lastSentAt: string | null = rule.last_sent_at ?? null;
