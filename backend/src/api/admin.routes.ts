...
export const adminRoutes = async (app: FastifyInstance) => {
  ...
  app.put("/admin/users/:id/role", { preHandler: requireAdmin }, async (request, reply) => {
    ...
    const result = db
      ...
      .run(parsed.data.role, now, (request.params as { id: string }).id);
    if (result.changes === 0) {
  ...
  app.post("/admin/push-rules", { preHandler: requireAdmin }, async (request, reply) => {
    ...
    db.prepare(
      ...
      parsed.data.lead_time_hours,
      parsed.data.target_user_id ?? null,
      parsed.data.target_role ?? null,
      parsed.data.title_template ?? null,
      parsed.data.body_template ?? null,
      parsed.data.min_response_percent ?? null,
      parsed.data.event_type ?? null,
      parsed.data.send_start ?? null,
      parsed.data.send_end ?? null,
      parsed.data.schedule_start_date ?? null,
      parsed.data.schedule_every ?? null,
      parsed.data.schedule_time ?? null,
      parsed.data.cooldown_hours ?? null,
      now,
      now
    );
  ...
  app.put("/admin/push-rules/:id", { preHandler: requireAdmin }, async (request, reply) => {
    ...
    const result = db
      .prepare(
        "UPDATE push_rules SET rule_type = ?, enabled = ?, lead_time_hours = ?, target_user_id = ?, target_role = ?, title_template = ?, body_template = ?, min_response_percent = ?, event_type = ?, send_start = ?, send_end = ?, schedule_start_date = ?, schedule_every = ?, schedule_time = ?, cooldown_hours = ?, updated_at = ? WHERE id = ?"
      )
      .run(
        parsed.data.rule_type,
        parsed.data.enabled ? 1 : 0,
  ...
  app.delete("/admin/push-rules/:id", { preHandler: requireAdmin }, async (request, reply) => {
    const result = db.prepare("DELETE FROM push_rules WHERE id = ?").run((request.params as { id: string }).id);
    if (result.changes === 0) {
      return reply.code(404).send({ error: "Not found" });
    }
