...
export const adminRoutes = async (app: FastifyInstance) => {
  ...
  app.post("/admin/push-rules", { preHandler: requireAdmin }, async (request, reply) => {
    ...
    db.prepare(
      ...
      parsed.rule_type,
      parsed.enabled ? 1 : 0,
      parsed.lead_time_hours,
      parsed.target_user_id ?? null,
      parsed.target_role ?? null,
      parsed.title_template ?? null,
      parsed.body_template ?? null,
      parsed.min_response_percent ?? null,
      parsed.event_type ?? null,
      parsed.send_start ?? null,
      parsed.send_end ?? null,
      parsed.schedule_start_date ?? null,
      parsed.schedule_every ?? null,
      parsed.schedule_time ?? null,
      parsed.cooldown_hours ?? null,
      now,
      now
    );
